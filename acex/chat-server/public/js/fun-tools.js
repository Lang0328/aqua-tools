...
// ----- 通用小说下载（搜书名 · 同源 /api/novel，版式同番茄下载器）-----
(function () {
    'use strict';
    const $ = (s) => document.querySelector(s);
    const root = $('#tool-novel');
    if (!root) return;
    ...
    async function searchBook() {
        ...
        if (!q) return;
        const box = $('#novelBooks');
        box.innerHTML = '<div class="fq-meta">搜索中…</div>';
        $('#novelDetail').style.display = 'none';
        currentBook = null; currentChapters = [];
        try {
            const r = await fetch('/api/novel/search?q=' + encodeURIComponent(q));
            if (!r.ok) throw new Error('HTTP ' + r.status);
            const books = await r.json();
            if (!Array.isArray(books) || !books.length) {
                box.innerHTML = '<div class="fq-meta">没有找到相关小说，换个书名试试</div>';
                return;
            }
            let html = '<div class="fq-meta">找到 ' + books.length + ' 本，点击选择：</div>';
            html += books.slice(0, 40).map((b, i) =>
                '<div class="fq-book" data-i="' + i + '">' +
                '<div class="fb-ico"><i class="fas fa-book"></i></div>' +
                '<div><div class="fb-title">' + esc(b.name) + '</div>' +
                '<div class="fb-author">' + esc(b.author || '未知') + '</div></div></div>'
            ).join('');
            box.innerHTML = html;
            box.querySelectorAll('.fq-book').forEach(el => {
                el.addEventListener('click', () => { currentBook = books[+el.dataset.i]; showBook(currentBook); });
            });
        } catch (e) {
            box.innerHTML = '<div class="fq-meta">搜索失败：' + esc(e.message) + '（请确认 server.js / novel-api 已启动）</div>';
        }
    }

    function showBook(b) {
        $('#novelDetail').style.display = 'block';
        $('#novelBookMeta').textContent = (b.name || '') + ' · ' + (b.author || '');
        $('#novelChapCount').textContent = '';
        $('#novelProgress').textContent = '';
        $('#novelLog').textContent = '';
        currentChapters = [];
    }

    async function loadChapters() {
        if (!currentBook) return;
        $('#novelChapCount').textContent = '加载章节中…';
        try {
            const r = await fetch('/api/novel/page?url=' + encodeURIComponent(currentBook.link));
            if (!r.ok) throw new Error('HTTP ' + r.status);
            const d = await r.json();
            currentChapters = Array.isArray(d.list) ? d.list : [];
            $('#novelChapCount').textContent = '共 ' + currentChapters.length + ' 章';
            if (d.desc) log('简介：' + d.desc.slice(0, 200));
        } catch (e) {
            $('#novelChapCount').textContent = '加载失败';
    ...
    async function downloadBook() {
        ...
        if (!currentChapters.length) { alert('请先点击「加载章节」'); return; }
        let list = currentChapters;
        const range = $('#novelRange').value;
        if (range !== 'all') list = list.slice(0, parseInt(range, 10));
        if (!list.length) return;
        const PER = 20;
        let txt = '《' + (currentBook.name || '未命名') + '》' + (currentBook.author ? '　作者：' + currentBook.author : '') + '\n\n';
        log('开始下载，共 ' + list.length + ' 章…');
        $('#novelDownload').disabled = true;
        try {
            for (let i = 0; i < list.length; i += PER) {
                const slice = list.slice(i, i + PER);
                const links = slice.map(c => c.link).join(',');
                try {
                    const r = await fetch('/api/novel/batch?links=' + encodeURIComponent(links));
                    const data = await r.json();
                    const strs = data.chapters || [];
                    for (let k = 0; k < strs.length; k++) {
                        const ch = slice[k];
                        txt += '\n' + (ch.name || '') + '\n\n' + (strs[k] || '') + '\n';
                    }
                    const done = Math.min(i + PER, list.length);
                    $('#novelProgress').textContent = '已获取 ' + done + ' / ' + list.length;
                    log('已获取 ' + done + ' / ' + list.length + ' 章');
                } catch (e) {
            ...
            a.download = (currentBook.name || 'novel') + '.txt';
            a.click();
            URL.revokeObjectURL(a.href);
            log('下载完成：' + (currentBook.name || 'novel') + '.txt');
        } finally {
            $('#novelDownload').disabled = false;
        }
    }

    // 仅 GitHub Pages 等纯静态托管没有本地后端，/api/novel 不存在，禁用并提示；
    // CloudStudio / 本地 localhost 等能跑后端的环境保持可用
    const isStaticPages = location.hostname.endsWith('.github.io');

    function showLocalOnlyNotice() {
        const notice = document.createElement('div');
        notice.className = 'fq-meta';
        notice.style.cssText = 'display:block;background:rgba(99,102,241,.12);border:1px solid rgba(99,102,241,.35);color:#c7d2fe;padding:12px 14px;border-radius:10px;margin-bottom:12px;line-height:1.65;white-space:pre-line';
        notice.innerHTML = '<i class="fas fa-info-circle"></i> <b>此功能为本地功能</b>\n' +
            '「小说下载」依赖本地 Node 服务（server.js + novel-api 后端），GitHub Pages 等纯静态托管无法运行后端，故在此不可用。\n' +
            '如需使用：在本机执行 <code>node server.js</code>，再访问 <code>http://localhost:8787</code>；或部署到支持运行 Node 后端的平台（如 Cloud Studio）。';
        const body = root.querySelector('.tool-body');
        if (body) body.insertBefore(notice, body.firstChild);
        const sb = $('#novelSearchBtn'); const inp = $('#novelSearch');
        if (sb) sb.disabled = true;
        if (inp) { inp.disabled = true; inp.placeholder = '需本地运行 server.js 后可用'; }
    }

    function init() {
        const sb = $('#novelSearchBtn'); if (!sb) return;
        if (isStaticPages) { showLocalOnlyNotice(); return; }
        sb.addEventListener('click', searchBook);
        $('#novelSearch').addEventListener('keydown', e => { if (e.key === 'Enter') searchBook(); });
        $('#novelLoadChapters').addEventListener('click', loadChapters);
        $('#novelDownload').addEventListener('click', downloadBook);
    }
